...
const isWordBounded = (text, startPos, length, segmenter) => {
    ...
    if (segmenter) {
        ...
        if (!startSegment)
            return false; // If the start index is inside a word segment but not the first character
        ...
        if (startSegment.isWordLike && startSegment.index != startPos)
            return false; // |endPos| points to the first character outside the target substring.
...
const BlockTextAccumulator = class {
    ...
    appendNode(node) {
        ...
        if (!isText(node))
            return; // Get the part of node inside the search range. This is to avoid
...
const findWordStartBoundInTextNode = (node, startOffset) => {
    ...
    // need to do anything.
    if (offset < node.data.length && internal.BOUNDARY_CHARS.test(node.data[offset]))
        return offset;
...
//# sourceMappingURL=fragment-generation-utils.js.map